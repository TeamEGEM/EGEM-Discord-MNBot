...
exports.run = (client, message, args) => {
  if(!message.member.hasPermission('ADMINISTRATOR')){
    return message.channel.send("You cannot use '/setcheck2' command");
  }
  con.getConnection(function(err, connection) {
    ...
    connection.query("SELECT * FROM data WHERE userId = ?", author, function (err, result) {
      ...
      connection.query(`UPDATE data SET shouldCheck2 = ? WHERE userId = ?`, [response, author]);
      ...
      const embed = new Discord.RichEmbed()
        ...
        .addField("IP Check #2 Updated to: ", response)
